 ...
 * seguendo delle regole. TODO spostare la logica di calcolo delle formule su un
 * altra classe poiche' condivisa da molti servizi
 * 
 * 
 * @author pascarelli
 * 
...
public class FormulaManager implements ISubscriber<JPAEvent>
{
    ...
    /**
     * Metodo che ascolta i messaggi che arrivano dal saveOrUpdate per
     * ricalcolare le formule riferite alla variabile che si e' editata.
     * 
     * @param variabileId
     *            l'id dell'oggetto da cercare nella lista di oggetti Variabile
     *            di ProprietaDaRicalcolare
     * @param classe
     *            la classe dell'oggetto da cercare nella lista di oggetti
     *            Variabile di ProprietaDaRicalcolare
     * @author pascarelli
    ...
    /**
     * Trasforma l'expression scritta dall'utente (salvata su db) in una regola
     * comprensibile a OGNL; REGOLE: - l'utente inserira' una espressione del
     * tipo '${costo}[1]', questa espressione verra' trasformata in un formato
     * leggibile a OGNL quindi diventera' 'anagrafica4view['costo'][1].object';
     * - la stringa '${costo}[n]'
     * diventera' 'anagrafica4view['costo'][n].object'; - ATT!!! se ho una combo
     * bisogna valutarla in modo tale che se la stringa immessa dall'utente sia
     * '${combo[${idx}].costo[1]*IVA}' bisogna valutare per prima la combo con
     * il suo idx passato come parametro per poi prendere il giusto valore
     * dell'elemento costo; - se non voglio utilizzare idx lo metto a null (ad
     * esempio quando voglio solamente parserizzare una stringa) - Per la
     * ripetibilita' nella formula bisogna inserire un resultIdx in modo tale da
     * sostituirlo con quello passato come parametro Es. se l'utente chiama
     * questo metodo con un'espressione del tipo opera.${test}[${resultIdx}] con
     * parametro resultIdx uguale a 4 allora il metodo cerchera' la 4 proprieta
     * ripetibile nell'anagrafica 4 view - Il metodo deve essere chiamato sempre
     * con parametro resultIdx uguale a 0 in modo tale da verificare se la
     * formula contiene o meno la stringa resultIdx nell'espressione e quindi
     * poter valutare o meno la ripetibilita'
     * 
     * @param idx
     *            indice di posizione della proprieta
     * @param resultIdx
     *            indice di posizione della proprieta ripetibile (se null cerca
     *            di restituire la lista di proprieta dell'anagrafica)
     * @return l'espressione da valutare
     */
    public static String getOgnlExpression(String formula, Integer idx,
            Integer resultIdx)
    {
        log.debug("Elaborazione della formula: " + formula + " con idx: " + idx
                + " in posizione: " + resultIdx);
        ...
        if (resultIdx != null)
        {
            ...
            {
                ognlExp = ognlExp.replaceAll("\\$\\{([A-z_]*)\\}\\[(\\d*)\\]",
                        "anagrafica4view['$1'][$2].object");
            }
        }
    ...
    /**
     * Valuta la regola di calcolo con il getValue di OGNL; La stringa passata
     * come parametro viene prima trasformata dal metodo
     * <em>getOgnlExpression</em>
     * 
     * @see getOgnlExpression
     * @param expression
     *            e' la formula
     * @param rootObject
     *            e' l'oggetto su cui valutare la formula
     * @param idx
     *            indice di posizione della proprieta
     * @author pascarelli
     * @throws OgnlException
    ...
    /**
     * Calcola il valore dell'espressione con il getValue di OGNL; La stringa
     * passata come parametro viene prima trasformata dal metodo
     * <em>getOgnlExpression</em>
     * 
     * @see getOgnlExpression
     * @param expression
    ...
    /**
     * Metodo di utilita' per la registrazione di formule appena create, itera
     * sull'anagrafica verificando se l'oggetto e' di tipo WidgetFormula, una
     * volta verificato questo registra la formula nel ProprietaDaRicalcolare.
     * 
     * @param oggetto
     *            di tipo AnagraficaSupport quindi contenente un anagrafica;
     * @throws OgnlException
     *             ;
     * @author pascarelli;
     */
    public <P extends Property<TP>, TP extends PropertiesDefinition> void registraFormule(
            AnagraficaSupport<P, TP> oggetto)
    {
        ...
        if (id != null)
        {
            applicationService.deleteOggettoDaRicalcolareByOggetto(id, oggetto
        ...
        for (TP tip : tipologieConRenderingFormula)
        {
            ...
            variabili.addAll(getEventiVariabili(
                    ((WidgetFormula) tip.getRendering()).getVariabili(),
                    oggetto, null));

        }

        // creo un model event per l'oggetto su cui si sta lavorando se ha
        // almeno una formula
        if (tipologieConRenderingFormula.size() > 0)
        {
            ModelEventoString mes = new ModelEventoString(oggetto.getClass()
                    .getCanonicalName(), ((Identifiable) oggetto).getId());
            variabili.add(mes);
        }
        ...
        {
            JPAEvent event = new JPAEvent(variabile.getEvento(),
                    variabile.getId(), JPAEvent.UPDATE);
            OggettoDaRicalcolare fe = new OggettoDaRicalcolare(id, oggetto
    ...
    public void receive(Class<JPAEvent> classeEvento, JPAEvent evento)
    {
        try
        {
            if (evento.getClazz().equals(
                    OggettoDaRicalcolare.class.getCanonicalName()))
            {
                return;
            }
            ...
            else
            {
                if (evento.getOperationType().equals(JPAEvent.CREATE))
                {
                    ...
                    try
                    {
                        object = applicationService.get(
                                Class.forName(evento.getClazz()),
                                evento.getId());
                    }
    ...
    private Set<ModelEventoString> getEventiVariabili(
            List<String> variabiliWidget, Object rootObject, Integer idx)
    {
        ...
        if (variabiliWidget != null && !variabiliWidget.isEmpty())
        {
            for (String variabile : variabiliWidget)
            {
                ...
                if (StringUtils.hasText(variabile))
                {
                    ...
                    try
                    {
                        Object result = FormulaManager.calcoloValore(variabile,
                                rootObject, idx, null);
                        ...
                        {
                            log.warn("Errore nel calcolo formula in metodo getEventiVariabili: il risultato della valutazione non e' Identifiable o una Collection");
                        }
    ...
    public void setApplicationService(
            IPersistenceFormulaService applicationService)
    {
        this.applicationService = applicationService;
    }
    ...
    class ModelEventoString
    {
        ...
        public int hashCode()
        {
            ...
            if (evento != null)
                hash.append(evento.hashCode());
            if (id != null)
                hash.append(id.hashCode());
