...
<%@ attribute name="treeObjectType" required="true" %>
<%@ attribute name="rootResearchObject" required="false" %>
<%@ attribute name="metadataBuilderTree" required="true" %>
<%@ attribute name="chooseOnlyLeaves" required="false" type="java.lang.Boolean" %>
...
<%-- FINE CODICE COMUNE A TUTTI I TAG... --%>
		...
		<span id="classificationtree_${id}_repeatable" class="spandatabind">${repeatable == true?'true':'false'}</span>			 
		<span id="classificationtree_${id}_path" class="spandatabind">${propertyName}</span>
		<span id="classificationtree_${id}_tot" class="spandatabind">${fn:length(values)}</span>
		<span id="classificationtree_${id}_treeObjectType" class="spandatabind">${treeObjectType}</span>
		<span id="classificationtree_${id}_rootResearchObject" class="spandatabind">${rootResearchObject}</span>
		<span id="classificationtree_${id}_metadataBuilderTree" class="spandatabind">${metadataBuilderTree}</span>
		<span id="classificationtree_${id}_chooseOnlyLeaves" class="spandatabind">${chooseOnlyLeaves}</span>
	    <span id="classificationtree_${id}_propertyPath" class="spandatabind">${propertyPath}</span>	
		<span class="spandatabind classificationtreeinfo">${id}</span>


		<a id="classificationtree_${id}_btn" class="btn btn-default">Expand tree!</a>
	
		<div id="classificationtree_${id}_selected">	
			<c:forEach var="value" items="${values}" varStatus="iterationStatus">	
				<spring:bind path="${propertyPath}[${iterationStatus.count - 1}]">
					...
					<div id="classificationtree_${id}_selected_${iterationStatus.count - 1}">
