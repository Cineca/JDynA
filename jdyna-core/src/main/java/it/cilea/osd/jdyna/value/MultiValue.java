...
public abstract class MultiValue<P extends Property<TP>, TP extends PropertiesDefinition>
		extends AValue<List<P>> implements AnagraficaSupport<P, TP> {
	@Transient
	 ...
	 * ha come chiave lo shortname della tipologia di proprieta' e come valore
	 * la lista di proprieta' corrispondenti.
	 * 
	 * @return Mappa key=shortname della tipologia di proprieta' : value=lista
	 *         di elementi dell'anagrafica (se alla stessa key si riferisce una
	 *         lista con piu' elementi vuol dire che la tipologia della
	 *         proprieta e' ripetibile)
	 */
	@Transient
	public Anagrafica4View<P, TP> getAnagrafica4view() {
		...
		for (P property : getObject()) {
			...
			if (appoggio == null) {
				...
				cacheAnagr4View
						.put(property.getTypo().getShortName(), appoggio);
			}
	...
	public String[] getTokenizedValue() {
		if (getObject() == null) {
			...
			for (P prop : getObject()) {
				if (prop.getTypo().isSimpleSearch() && prop.getValue() != null) {
	...
	public P createProprieta(TP tipologiaProprieta) {
		return createProprieta((P)this.getAnagrafica().get(0).getPropertyParent(), tipologiaProprieta);
	}
	...
	public P createProprieta(TP tipologiaProprieta, Integer posizione) {		
		return createProprieta((P)this.getAnagrafica().get(0).getPropertyParent(), tipologiaProprieta, posizione);
	}
	...
	public P createProprieta(P proprieta, TP tipologiaProprieta) {
		if (log.isDebugEnabled()) {
			log.debug("Creazione della sottoproprieta' " + tipologiaProprieta
					+ " in " + proprieta);
			log.debug("P-TP: " + proprieta.getTypo().getShortName() + " P-V: "
					+ proprieta.getValue().getObject());
		}
		// FIXME aggiungere il check delle precondizioni (TP sottotipologia
		// ammessa in P)
		// e' nella combo che devo calcolare la posizione delle proprieta di una
		// certa tipologia
		List<P> propList = getProprietaDellaTipologiaInValoreMulti(
				(MultiValue<P, TP>) proprieta.getValue(), tipologiaProprieta);
	...
	public P createProprieta(P proprieta, TP tipologiaProprieta,
		...
		if (log.isDebugEnabled()) {
			...
			log.debug("P-TP: " + proprieta.getTypo().getShortName() + " P-V: "
					+ proprieta.getValue().getObject());
			log.debug("TP: " + tipologiaProprieta.getShortName() + " Pos: "
					+ posizione);
		}
		List<P> propList = getProprietaDellaTipologiaInValoreMulti(
				(MultiValue<P, TP>) proprieta.getValue(), tipologiaProprieta);
	...
	public boolean removeProprieta(P proprieta) {
		if (log.isDebugEnabled()) {
			log.debug("Rimozione della proprieta': " + proprieta
					+ " dall'oggetto: " + proprieta.getParent() == null ? proprieta
					.getPropertyParent().getParent() : proprieta.getParent()
		...
		if (proprietaParent == null) {
			...
			// non e' necessario perche' il getProprietaDellaTipologia
			// restituisce
			// una referenza
			...
			rimossa = ((MultiValue<P, TP>) proprietaParent.getValue())
					.getObject().remove(proprieta);
			...
			propList = getProprietaDellaTipologiaInValoreMulti(
					(MultiValue<P, TP>) proprietaParent.getValue(),
					proprieta.getTypo());
			int pos = proprieta.getPosition();
			int numProp = propList.size();
			// proprieta.setParent(null); // 0 1 2
			// bisogna partire da pos+1 perche' e' vero che dalla anagrafica e'
			// gia'
	...
	private boolean removeSottoProprieta(P proprieta) {
		...
		boolean rimossa = false;
		try {
			MultiValue<P, TP> valoreMulti = ((MultiValue<P, TP>) proprieta
					.getValue());
			...
			for (P proprietaFiglia : sottoProprieta) {
				...
				// proprietaFiglia.setParent(null); // necessaria per cancellare
				// la proprieta' dal db
	 ...
	 * {@inheritDoc}
	 */
	public List<P> getProprietaDellaTipologiaInValoreMulti(
			MultiValue<P, TP> valoreMulti, TP tipologiaProprieta) {
		return valoreMulti.getAnagrafica4view().get(
	...
	@SuppressWarnings("unchecked")
	private P createProprietaWithoutPosizione(P proprietaParent,
		...
		// altrimenti l'aggiungo solo alla proprieta parent
			...
			if (proprietaParent != null) {
				...
				((MultiValue<P, TP>) proprietaParent.getValue()).getObject()
						.add(proprieta);
			}
		}
		...
		if (tipologiaProprieta.getRendering() instanceof WidgetCombo) {
			...
			for (TP tp : ((WidgetCombo<P, TP>) tipologiaProprieta
